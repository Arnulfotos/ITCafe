...
do
{
    ...
    // Ruta del archivo
    string filePath = "c:/ITCafe/menu_Itstep_cafeteria.txt";
    ...
    Console.WriteLine(String.Format("| {0,-3} {1,-18} {2,-60} {3,-5} |", "", "", "Menu", ""));
    Console.WriteLine("-----------------------------------------------------------------------------------------");
    foreach (string[] product in menu)
    {
        Console.WriteLine(String.Format("| {0,-5} | {1,-18} | {2,-60} | {3,-5} |", counter, $"{product[0]}", product[1], product[2]));
        counter++;
    }
    ...
    {
        Console.WriteLine(elemento[0]);
        total += int.Parse(elemento[2].Substring(1));
    }
